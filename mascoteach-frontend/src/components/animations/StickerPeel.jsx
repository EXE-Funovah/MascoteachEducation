...
const StickerPeel = ({
  ...
  useEffect(() => {
    const target = dragTargetRef.current;
    const boundsEl = target.closest('section') || target.parentNode;
  ...
  const cssVars = useMemo(
    () => ({
      ...
      '--sticker-drop-shadow-filter': `url(#${filterId.dropShadow})`,
      '--sticker-point-light-filter': `url(#${filterId.pointLight})`,
      '--sticker-point-light-flipped-filter': `url(#${filterId.pointLightFlipped})`,
      '--sticker-expand-fill-filter': `url(#${filterId.expandAndFill})`,
    }),
    [rotate, peelBackHoverPct, peelBackActivePct, peelEasing, peelHoverEasing, width, shadowIntensity, lightingIntensity, peelDirection, filterId.dropShadow, filterId.pointLight, filterId.pointLightFlipped, filterId.expandAndFill],
  );

  return (
    <div className={`sticker-draggable ${className}`} ref={dragTargetRef} style={{ ...cssVars, ...style }}>
