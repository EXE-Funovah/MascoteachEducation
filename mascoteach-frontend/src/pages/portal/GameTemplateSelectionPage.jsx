...
export default function GameTemplateSelectionPage() {
  ...
  const [playingDirectly, setPlayingDirectly] = useState(false);

  async function handlePlayDirect() {
    if (!quizId) return;
    setPlayingDirectly(true);
    try {
      const raw = await getQuestionsByQuiz(quizId);
      const questions = Array.isArray(raw) ? raw.map(normalizeQuestion) : [];
      if (!questions.length) {
        setCreateError("Bộ câu hỏi này chưa có câu hỏi nào!");
        return;
      }
      navigate("/play/adventure", {
        state: {
          session: { id: 0, quizId, pin: "PREVIEW" },
          questions,
        },
      });
    } catch (err) {
      setCreateError(err.message || "Không thể tải câu hỏi.");
    } finally {
      setPlayingDirectly(false);
    }
  }
  ...
  async function handleCreateSession() {
    ...
    try {
      ...
      setCreateError(
        err.message || "Không thể tạo phiên chơi. Vui lòng thử lại.",
      );
      setCreating(false);
    }
  }
          ...
          <div className="flex flex-col items-center justify-center py-12 text-center gap-4">
            ...
            <div>
              <p className="text-[14px] font-semibold text-slate-600 mb-1">
                Chưa có template trong hệ thống
              </p>
              <p className="text-[13px] text-slate-400">
                Bạn vẫn có thể chơi thử trực tiếp từ bộ câu hỏi
              </p>
            </div>
            <motion.button
              onClick={handlePlayDirect}
              disabled={playingDirectly}
              className="flex items-center gap-2 px-6 py-3 rounded-xl
                                       bg-gradient-to-r from-violet-500 to-purple-500 text-white
                                       text-[14px] font-bold shadow-md hover:from-violet-600 hover:to-purple-600
                                       disabled:opacity-60 disabled:cursor-not-allowed transition-all"
              whileHover={!playingDirectly ? { scale: 1.03, y: -1 } : {}}
              whileTap={!playingDirectly ? { scale: 0.97 } : {}}
            >
              {playingDirectly ? (
                <>
                ...
                <>
                  <Play className="w-4 h-4" /> Chơi thử ngay 🎮
                </>
              )}
            </motion.button>
            <button
          ...
          <>
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-8">
              {gameTemplates.map((template) => {
                const IconComponent =
                  modeIcons[template.icon] || getIconForTemplate(template.name);
            ...
            <div className="flex items-center justify-between pt-4 border-t border-slate-100/60">
              ...
              <motion.button
                ...
                whileHover={
                  !creating && selectedTemplate ? { scale: 1.02, y: -1 } : {}
                }
